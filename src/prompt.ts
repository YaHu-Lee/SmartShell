...
3. 复杂问题可能需要经过多轮工具调用
4. 当你需要向用户确认信息时，只能通过 userInput 工具，禁止通过其他方式向用户询问信息
5. 同一个工具，同一个参数，不能调用超过 3 次。若你发现循环调用仍无法判断问题是否解决，请立刻停止并告知用户当前无法解决该问题
...
接收一个字符串，将这个字符串以 utf-8 编码写入到文件中。当你需要将文本内容写入到文件时，请优先考虑使用这个工具，不要通过直接执行 shell 命令来完成，因为直接执行 shell 命令会导致乱码
`;

export const deleteFileToolPrompt = `
接收一个字符串，将这个字符串作为文件路径，删除这个文件。当你需要删除文件时，**必须** 使用这个工具，不能使用其他方式删除文件！！！重要！！！。
`;
